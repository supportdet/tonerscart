...
- **"Couldn't find your toner?" product-request form** (`components/ProductRequestForm.jsx`): card + dialog placed at the bottom of `Search.jsx` (toner), `Papers.jsx` (paper), `Consumables.jsx` (consumable), `PrintersResults.jsx` (printer). Fields: Product*, Email*, Phone (optional, +91), City (optional, prefilled from CityContext) → `POST /api/mps/inquiry` with `selections.type="product_request"` + category → mps_inquiries + email. Confirmed appears in Admin → Messages (`msg_type:"product_request"`, full selections) and emails support.
- **Backend** (`email_service.email_mps_inquiry`): added `product_request` and `feedback` subject/heading branches (generic `sel_rows` table renders the extra fields). No new endpoint — reuses the existing `/api/mps/inquiry` which already does DB insert + email.
- **Checkout mobile** (`pages/Checkout.jsx`): no horizontal overflow was reproducible (global `overflow-x: clip` + `.tc-hero overflow:hidden` already handle it); added canonical `min-w-0` guards to the 3 grid columns (form / summary / aside) to prevent any future grid blow-out from non-shrinkable children. Verified scrollWidth==clientWidth==390 on both steps.
- Tested: curl (both enquiry types → {ok:true}), admin messages GET (row surfaced), Playwright (tab visibility, both dialogs open, no overflow @390 & @1280). Test rows cleaned from mps_inquiries. Constraints honored: no CORS change, no emergentintegrations, Razorpay/Twilio untouched.
