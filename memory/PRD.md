...
> **Latest (2026-06-11c):** (1) **Cartridge SVG redesigned again to match user's reference**: horizontal cartridge wider than tall, cylindrical housings on BOTH ends, rectangular middle body, grip at bottom center, light grey/white plastic with subtle shading, brand name in white on an ALWAYS-RED band across the front face (brand-specific accent colors removed per instruction); (2) **All test data deleted**: users qadealer@tonerscart.in + e2e_deliv_dealer/buyer@tonerscart.in (users + suppliers + Supabase Auth), 1 test scanner listing (Canon LiDE 400), 3 test orders — DB now contains only real registrations; NO QA supplier account exists anymore (create+delete one for future supplier-side testing); (3) **Finder popup robustness fix**: clicks on cookie banner/header no longer suppress the 15s popup (pointerdown listener scoped to the printers page container; scroll threshold 120px). Verified via Playwright: popup opens at 15s idle, X closes it. NOTE: popup shows once per browser session (sessionStorage `tc_finder_popup_shown`).
>
